...
public class DataSqlAnalyzer {
	...
	public static void main(String[] args) throws Exception {
		...
		Table table = tenv.sqlQuery("select 保障工单类型,参数值 from data group by 保障工单类型,参数值");
		tenv.toDataSet(table, DataSqlResult.class)
			.map(new DataMapFunction())
			.writeAsCsv("D:\\数据分析\\20181102\\data99_结果2.csv", WriteMode.OVERWRITE)
			.setParallelism(1);
