...
/**
 * 来自：
* https://kaiwu.lagou.com/course/courseInfo.htm?courseId=614&wxinfo=2#/detail/pc?id=6437
* 
 */

public class FlinkRiskEngine {

	private static final List<String[]> features = Arrays.asList(parseDSL("count(pay_account.history,1h)"),
			parseDSL("sum(amount#rcv_account.history,1h)"), parseDSL("count_distinct(rcv_account#pay_account.history,1h)"));

	private static String[] parseDSL(String dsl) {
		return Arrays.stream(dsl.split("[(,)]")).map(String::trim).collect(Collectors.toList()).toArray(new String[0]);
	}
	...
	public static void main(String[] args) throws Exception {
		...
		String[] datas = mockData();
		DataStream<String> stream = env.fromElements(datas);

		DataStream counts = stream.map(new MapFunction<String, JSONObject>() {
	...
	public static String[] mockData() {
        int samples = 1000;
        String[] datas = new String[samples];
	...
	public static class FeatureEnrichFunction extends RichMapFunction<JSONObject, JSONObject> {
		...
		private double doSum(JSONArray history, String target, long window) {
			long maxTimestamp = history.stream().map(x -> ((JSONObject) x).getLong("timestamp")).max(Long::compare).orElse(Long.MIN_VALUE);
			long minTimestamp = maxTimestamp - window;
			return history.stream().filter(x -> ((JSONObject) x).getLong("timestamp") > minTimestamp).map(x -> ((JSONObject) x).getDouble(target))
		...
		private double doCount(JSONArray history, String target, long window) {
			long maxTimestamp = history.stream().map(x -> ((JSONObject) x).getLong("timestamp")).max(Long::compare).orElse(Long.MIN_VALUE);
			long minTimestamp = maxTimestamp - window;
			return history.stream().filter(x -> ((JSONObject) x).getLong("timestamp") > minTimestamp).count();
		}

		private double doCountDistinct(JSONArray history, String target, long window) {
			long maxTimestamp = history.stream().map(x -> ((JSONObject) x).getLong("timestamp")).max(Long::compare).orElse(Long.MIN_VALUE);
			long minTimestamp = maxTimestamp - window;
			return history.stream().filter(x -> ((JSONObject) x).getLong("timestamp") > minTimestamp).map(x -> ((JSONObject) x).getString(target))
					.distinct().count();
		}
	}
	...
	private static FlinkKafkaConsumer<String> createKafkaConsumer() {
		...
		FlinkKafkaConsumer<String> consumer010 = new FlinkKafkaConsumer<String>("event-input", new SimpleStringSchema(), properties);
